...
> = Interfaces extends `${infer Name}${"," | "&"}${infer Rest}`
    ? MergeInterface<Rest, Known, Types & (Trim<Name> extends infer Key extends keyof Known ? Known[Key] : never)>
    : Types & (Trim<Interfaces> extends infer Key extends keyof Known ? Known[Key] : never)

export type RemoveComment<T extends string> = RemoveDocumentComment<RemoveMultiLineComment<RemoveSingleLineComment<T>>>

type RemoveSingleLineComment<T extends string> = T extends `${infer First}#${infer Comment}\n${infer Rest}`
    ? `${First}${RemoveSingleLineComment<Rest>}`
    : T

type RemoveDocumentComment<T extends string> = T extends `${infer First}"${infer Comment}"\n${infer Rest}`
    ? `${First}${RemoveDocumentComment<Rest>}`
    : T

type RemoveMultiLineComment<T extends string> = T extends `${infer First}"""${infer Comment}"""${infer Rest}`
    ? `${First}${RemoveMultiLineComment<Rest>}`
    : T

export type CreateInnerMobius<T extends string, Known extends CustomTypes = {}> = T extends `${infer Ops}{${infer Schema}}${infer Rest}`
                      ...
                      ? {
                            [name in TrimLeft<FirstWord<Name>>]: Prettify<
                                MapSchema<RemoveComment<Schema>, Known> &
                                    (Name extends `${infer _} implements ${infer Interfaces}` ? MergeInterface<Interfaces, Known> : {})
                            >
                        }
                          ...
                          : Keyword extends "fragment"
                            ...
                            : Keyword extends "directive"
                              ? CreateInnerMobius<
                                    ...
                                    `${TrimLeft<GetLastLine<Ops>>}{${RemoveComment<Schema>}}`,
                                    Known
                                >
                              : Keyword extends "union"
                                ...
                                : /**
                                  ...
                                  ? TrimLeft<RemoveComment<Ops>> extends `${infer _}\n${infer Prefix}`
...
type MapInnerUnion<T extends string[], Known extends CustomTypes, Carry extends Record<string, unknown> = {}> = T extends [
    ...
    ? MapInnerUnion<
          Rest,
          Known,
          Prettify<Known extends { [a in First]: infer A } ? (A extends string | boolean | number | symbol ? Carry | A : Carry & A) : Carry>
      >
    : Carry
...
type MapEnum<T extends string, Carry extends string | null = null> = T extends `${infer Name}${Whitespace | ","}${infer Rest}`
    ? MapEnum<Rest, Trim<Name> | Carry>
    : T extends `${infer Name}`
      ? Carry | Trim<Name>
      : Carry

type MapSchema<T extends string, Known extends CustomTypes = {}> = T extends `${infer Name}:${infer Type}`
    ? Name extends `${infer Name}(${infer Params}`
        ? T extends `${infer Name}(${infer Params}):${infer Type}`
            ? ExtractType<Type> extends [infer Type extends string, infer Rest extends string]
                ? {
                      [word in Name as TrimLeft<Name> extends infer Candidate extends string
                          ? Candidate extends `#${infer _}`
                              ? never
                              : Candidate
                          : never]: Prettify<MapArgument<Params, Known>> extends infer Argument
...
export type CreateQuery<T extends Record<string, unknown>> = (NonNullable<T> extends infer T
    ? {
          [K in keyof T]: T[K] extends (_: infer Params) => infer Query
              ? NonNullable<UnwrapArray<NonNullable<Query>>> extends Record<string, unknown>
                  ? NonNullable<UnwrapArray<Query>> extends infer A extends Record<string, unknown>
                      ? {
              ...
              : NonNullable<UnwrapArray<T[K]>> extends infer Query extends Record<string, unknown>
...
type UnwrapFunctionalSchema<Schema extends Record<string, unknown> | Function | null> = Schema extends (...p: any[]) => infer Returned
    ? NonNullable<UnwrapArray<Returned>> extends infer A extends Record<string, unknown>
        ? A
        : Returned
...
>(params: {
    ...
    Prettify<
        ({} extends Query ? {} : Resolve<Query, TypeDefs["Query"] & Scalars>) &
            ({} extends Mutate ? {} : Resolve<Mutate, TypeDefs["Mutation"] & Scalars>) &
            ({} extends Subscription ? {} : Resolve<Subscription, TypeDefs["Subscription"] & Scalars>)
    >
>
 ...
 */
export const mobiusToGraphQL = <T extends "query" | "mutation" | "subscription">(type: T, params: Record<T, Record<string, unknown>>) => {
    const query = JSON.stringify(
        params[type] ?? params,
        (key, value) => {
            if (typeof value !== "object") return value

            if (typeof value === "object" && "select" in value && !("where" in value)) return value.select
...
export class Mobius<
    ...
    TypeDefs extends CreateMobius<Declaration, Scalars> = CreateMobius<Declaration, Scalars>,
> {
    /**
